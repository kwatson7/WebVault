...
public class EncryptionTests extends AndroidTestCase {
	...
	public void testEncryptByteArrayToFile() throws Throwable{
		SymmetricEncryptor encryptor = new SymmetricEncryptor(passwordCorrect);
		byte[] byteArray = ("test byte").getBytes();
		encryptor.encryptByteArrayToFile(byteArray, encryptFile);
		byte[] byteArray2 = encryptor.decryptFileToByteArray(encryptFile, false);
		Assert.assertTrue(Arrays.equals(byteArray, byteArray2));
	}

	public void testEncryptByteArrayToByteArray() throws Throwable{
		SymmetricEncryptor encryptor = new SymmetricEncryptor(passwordCorrect);
		byte[] byteArray = ("test byte").getBytes();
		byte[] encrypted = encryptor.encryptByteArrayToByteArray(byteArray);
		byte[] byteArray2 = encryptor.decryptByteArrayToByteArray(encrypted);
		Assert.assertTrue(Arrays.equals(byteArray, byteArray2));
	}

	public void testEncryptByteArrayToBase64String() throws Throwable{
		SymmetricEncryptor encryptor = new SymmetricEncryptor(passwordCorrect);
		byte[] byteArray = ("test byte").getBytes();
		String encrypted = encryptor.encryptByteArrayToBase64String(byteArray);
		byte[] byteArray2 = encryptor.decryptBase64StringToByteArray(encrypted);
		Assert.assertTrue(Arrays.equals(byteArray, byteArray2));
	}

	public void testEncryptStringToString() throws Throwable{
		SymmetricEncryptor encryptor = new SymmetricEncryptor(passwordCorrect);
		String input = "test";
		String encryptedString = encryptor.encryptStringToBase64String(input);
		String decryptedString = encryptor.decryptBase64StringToString(encryptedString);
		Assert.assertTrue(input.equals(decryptedString));
	}

	public void testEncryptStringToByteArray() throws Throwable{
		SymmetricEncryptor encryptor = new SymmetricEncryptor(passwordCorrect);
		String input = "test";
		byte[] encryptedByte = encryptor.encryptStringToByteArray(input);
		String decryptedString = encryptor.decryptByteArrayToString(encryptedByte);
		Assert.assertTrue(input.equals(decryptedString));
	}
